   ...
   foreach month in 1 2 6 {
      ...
      * The first 7 are the best matches
      ...
      if `month'==1 {
         ...
         tex \item Persons with two or more tests showing z-score of \(-`threshold'\) or lower meet criteria for POCD.
         tex \item Persons who have a sum of 7 z-scores of \(-`threshold'\) or lower meet criteria for POCD.
         tex \end{enumerate}
         
         if "`threshold'"~="1.96" {
            tex Note: the original threshold proposed was 1.96, not `threshold' as described above. If we use a value
            tex other than 1.96 then we are conducting a sensitivity analysis, of sorts.\\[0.25cm]
         }
         ...
         tex z-scores indicating greater than `threshold' decline in neuropsychological test performance as 
         tex an condition for ruling in POCD. If we used 10 SAGES tests, it'd be easier to get to 
         tex the sum of \(-`threshold'\) or less. This discrepancy in tests rules out confident replication 
      ...
      * sum of low scores
      ...
      gen pocd=(sumz<-`threshold'|sumlow>=2) if sumz~=.&sumlow~=.
      table pocd
      ...
      * {{5, 6}, {5, 8}, {5, 9}, {5, 10}, {6, 8}, {6, 9}, {6, 10}, {8, 9}, {8, 10}, {9, 10}}
      ...
      forvalues i=0/9 {
         ...
         gen pocd`i'=(sumz`i'<-`threshold'|sumlow`i'>=2) if sumz`i'~=.&sumlow`i'~=.
