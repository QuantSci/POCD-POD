   ...
   foreach month in 1 2 6 12 {
      ...
      * The first 7 are the best matches
      ...
      if `month'==1 {
         ...
         tex \item Persons with two or more tests showing z-score of \(-1.64\) or lower meet criteria for POCD.
         tex \item Persons who have a sum of 7 z-scores of \(-1.64\) or lower meet criteria for POCD.
         tex \end{enumerate}
         ...
         tex z-scores indicating greater than 1.64 decline in neuropsychological test performance as 
         tex an condition for ruling in POCD. If we used 10 SAGES tests, it'd be easier to get to 
         tex the sum of \(-1.64\) or less. This discrepancy in tests rules out confident replication 
      ...
      * sum of low scores
      ...
      gen pocd=(sumz<-1.64|sumlow>=2) if sumz~=.&sumlow~=.
      table pocd
      ...
      * {{5, 6}, {5, 8}, {5, 9}, {5, 10}, {6, 8}, {6, 9}, {6, 10}, {8, 9}, {8, 10}, {9, 10}}
      ...
      forvalues i=0/9 {
         ...
         gen pocd`i'=(sumz`i'<-1.64|sumlow`i'>=2) if sumz`i'~=.&sumlow`i'~=.
